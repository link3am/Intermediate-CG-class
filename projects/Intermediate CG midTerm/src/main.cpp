...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_texSw", texSW);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			...
			{
				ImGui::SliderInt("Chosen Effect", &activeEffect, 0, 4);

				if (activeEffect == 0)
				{
					ImGui::Text("no lighting");

					PostEffect* temp = (PostEffect*)effects[0];
					lightAmbientPow = 0.0f;
					lightSpecularPow = 0.0f;
					band = 1;
					edgeChange = 0;
					lightCol = glm::vec3(0.0f, 0.0f, 0.0f);
					shader->SetUniform("u_LightCol", lightCol);
					shader->SetUniform("u_AmbientLightStrength", lightAmbientPow);
					shader->SetUniform("u_SpecularLightStrength", lightSpecularPow);
					shader->SetUniform("u_bands", band);
					shader->SetUniform("u_edgeChange", edgeChange);
				}
				if (activeEffect == 1)
				{
					ImGui::Text("ambient lighting only ");

					PostEffect* temp = (PostEffect*)effects[0];
					lightAmbientPow = 0.05f;
					lightSpecularPow = 0.0f;
					band = 1;
					edgeChange = 0;
					lightCol = glm::vec3(0.9f, 0.85f, 0.5f);
					shader->SetUniform("u_LightCol", lightCol);
					shader->SetUniform("u_AmbientLightStrength", lightAmbientPow);
					shader->SetUniform("u_SpecularLightStrength", lightSpecularPow);
					shader->SetUniform("u_bands", band);
				...
				if (activeEffect == 2)
				{
					ImGui::Text(" specular lighting only");
					PostEffect* temp = (PostEffect*)effects[0];
				...
				if (activeEffect == 4)
				{
					...
					BloomEffect* temp = (BloomEffect*)effects[1];
			...
			if (ImGui::Button("tex on", ImVec2(200.0f, 40.0f)))
			{
				texSW = 1;
				shader->SetUniform("u_texSw", texSW);
			}
			if (ImGui::Button("tex off", ImVec2(200.0f, 40.0f)))
			{
				texSW = 0;
				shader->SetUniform("u_texSw", texSW);
			}
		...
		Texture2D::sptr tester = Texture2D::LoadFromFile("images/tester.png");
		...
		Texture2D::sptr donutTex = Texture2D::LoadFromFile("images/donut UV.png");
		Texture2D::sptr chessTex = Texture2D::LoadFromFile("images/chess UV.png");
		...
		donutMat->Shader = shader;
		donutMat->Set("s_Diffuse", donutTex);
		...
		chessMat->Shader = shader;
		chessMat->Set("s_Diffuse", chessTex);
		...
		grassMat->Set("u_Shininess", 20.0f);
		...
		GameObject obj2 = scene->CreateEntity("donut");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/donut.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(donutMat);
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, 2.0f);
			obj2.get<Transform>().SetLocalRotation(0.0f, 0.0f, 0.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("chess");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/chess.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(chessMat);
			obj3.get<Transform>().SetLocalPosition(0.0f, 0.0f, 4.0f);
	...
	//	effects.push_back(greyscaleEffect);
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			obj3.get<Transform>().SetLocalPosition(LERP(glm::vec3(-3.0, -2.0, 4.0), glm::vec3(3.0, -2.0, 4.0), lerpTimer));
			//lerp end
			...
			lightPos = cameraObject.get<Transform>().GetLocalPosition();
			shader->SetUniform("u_LightPos", lightPos);
			...
			if (activeEffect == 4) {
				effects[1]->ApplyEffect(basicEffect);
				effects[1]->DrawToScreen();
			}
			else
