...
//Toon shading
uniform int u_bands;
uniform float u_edgeChange;
uniform int u_texSw = 1;
...
void main() {
	...
	//if(dist==0){
	//	diffuse = floor(diffuse / dist)*scaleFactor;
	//}
	...
	//outline effect
	float edge = (dot(viewDir, N) < u_edgeChange) ? 0.0 : 1.0;
	...
	if(u_texSw ==1)
	{
	frag_color = vec4(result*edge, textureColor.a);
	}
	if(u_texSw == 0)
	{
	frag_color = vec4(1);
