...
void BloomEffect::Init(unsigned width, unsigned height)
{
    ...
    _shaders[index]->LoadShaderPartFromFile("shaders/Post/gaos_frag.glsl", GL_FRAGMENT_SHADER);
    _shaders[index]->Link();
}
...
void BloomEffect::ApplyEffect(PostEffect* buffer)
{
    ...
    _shaders[0]->SetUniform("u_blurMod", _blurMod);
...
float BloomEffect::GetBlurmod() const
{
    return _blurMod;
}
...
void BloomEffect::SetBlurmod(float mod)
{
    _blurMod = mod;
}
