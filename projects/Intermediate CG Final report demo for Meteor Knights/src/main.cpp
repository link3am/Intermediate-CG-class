...
#pragma region Shader and ImGui 
	...
	ColorCorrectionEffect* colorCorrectEffect;
	...
	BackendHandler::imGuiCallbacks.push_back([&]() {
		if (ImGui::CollapsingHeader("toggles "))
		{
			...
			if (activeEffect == 4)
			{
				...
				int blurmod = temp->GetBlurmod();
				if (ImGui::SliderInt("mod", &blurmod, 0, 2))
				{
					temp->SetBlurmod(blurmod);
				}
				if (ImGui::SliderFloat("Intensity", &intensity, 0.0f, 1.0f))
				{
			...
			if (activeEffect == 6)
			{
				...
				if (ImGui::SliderInt("warm/cold", &iswarm, 0.0f, 1.0f))
				{
					if (iswarm == 0) {
						temp->SetLUT(warmCube);
					}
					else {
						temp->SetLUT(coldCube);
					}
				}
			}
		}
...
// We need to tell our scene system what extra component types we want to support
	...
	{
		colorCorrectEffect = &colorCorrectEffectObject.emplace<ColorCorrectionEffect>();
