...
void BackendHandler::GlfwWindowResizedCallback(GLFWwindow* window, int width, int height)
{
	...
	Application::Instance().ActiveScene->Registry().view<PixelEffect>().each([=](PixelEffect& buf)
		{
			buf.Reshape(width, height);
		});
	...
	Application::Instance().ActiveScene->Registry().view<FilmEffect>().each([=](FilmEffect& buf)
		{
			buf.Reshape(width, height);
		});
}
