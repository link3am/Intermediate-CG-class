...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		LUT3D cusCube("cubes/cus.cube");
		LUT3D coldCube("cubes/cold.cube");
		LUT3D warmCube("cubes/warm.cube");
		PostEffect* basiceffect;
		std::vector<PostEffect*>effects;
		std::vector<LUT3D*>colors;
		int activeEffect = 0;
		int activeColor = 0;
		GreyscaleEffect* greyscaleEffect;		
		SepiaEffect* sepiaeffect;
		ColorCorrectionEffect* colorCorrect;
		ColorCorrectionEffect* coldColor;
		ColorCorrectionEffect* warmColor;
		// We'll add some ImGui controls to control our shader
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effect controls")) {
				ImGui::SliderInt("Chosen Effect", &activeEffect, 0, 5);
				
				if (activeEffect == 0)
				{
					ImGui::Text("Active Effect: Color Correction effect");
				...
				if (activeEffect == 1)
				{
					...
					PostEffect* temp = (PostEffect*)effects[1];
			...
			if (ImGui::CollapsingHeader("color controls")) {
				ImGui::SliderInt("color", &activeColor, 0, 2);
		...
		GameObject obj2 = scene->CreateEntity("monkey_quads");
		{
			...
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, 22.0f);
			obj2.get<Transform>().SetLocalRotation(0.0f, 0.0f, -90.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}
		GameObject melonObj = scene->CreateEntity("melon");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/watermelon.obj");
			melonObj.emplace<RendererComponent>().SetMesh(vao).SetMaterial(melonMat);
			melonObj.get<Transform>().SetLocalPosition(0.0f, 0.0f, 2.0f);
			melonObj.get<Transform>().SetLocalRotation(90.0f, 0.0f, 90.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(melonObj);
		}
		...
		*/
		GameObject framebufferObject = scene->CreateEntity("basiceffect");
		{
			basiceffect = &framebufferObject.emplace<PostEffect>();
			basiceffect->Init(width, height);
		}
		GameObject colorCorrectionOBJ = scene->CreateEntity("color");
		{
			colorCorrect = &colorCorrectionOBJ.emplace<ColorCorrectionEffect>();
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			obj2.get<Transform>().SetLocalPosition(LERP(glm::vec3(-3.0, -2.0, 1.0), glm::vec3(3.0, -2.0, 1.0), lerpTimer));
			//lerp end
			...
			if (activeEffect == 0) {
				effects[0]->ApplyEffect(basiceffect);
